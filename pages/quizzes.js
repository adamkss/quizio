...
import {
  deleteQuiz,
  getQuizzesOfCurrentUser,
  createNewGenericQuiz,
} from "../utils/QuizRequests";
...
const Quizzes = () => {
  ...
  const [
    ...
    setIsCreatingNewDialogInProgress,
  ] = useState(false);
  ...
  const onCreateNewQuiz = useCallback(
    async (newQuizName) => {
      setIsLoading(true);
  ...
  const getDeleteQuizCallback = useCallback(
    (quizId) => async () => {
  ...
  const getOnSeeResultsOfQuizClickCallback = useCallback(
    (quizId) => () => {
          ...
          <section className="quizes">
            {quizes.map((quiz) => (
              <div className="quiz" key={quiz.id}>
                <h2>{quiz.name}</h2>
                <button
                  className="quiz-operation-button"
                  onClick={getOnClickCallbackForQuizPress(quiz.id)}
                >
                  Open quiz editor
                </button>
                <button
                  className="quiz-operation-button"
                  onClick={getOnSeeResultsOfQuizClickCallback(quiz.id)}
                >
