...
import { Grid, GridElement } from "react-simple-grid-dnd";
...
const getQuestionsForReplacedQuestionOptionsOfQuestion = (
  questions,
  questionId,
  newQuestionOptions
) => {
...
const getQuestionsForReplacedQuestion = (
  questions,
  questionId,
  newQuestion
) => {
...
const Editor = () => {
  ...
  const [
    ...
    setIsCreatingQuestionsInProgress,
  ] = useState(false);
  ...
  const reloadQuestion = React.useCallback(
    async (questionId) => {
      ...
      setQuestions(
        getQuestionsForReplacedQuestion(questions, questionId, newQuestion)
      );
    },
    [questions]
  );
  ...
  const onSaveQuestion = React.useCallback(
    async (title, options) => {
      await executeAsyncFunctionAndObserveState(
        setIsLoadingInProgress,
        createQuestion,
  ...
  const getOnAddNewOptionCallback = React.useCallback(
    (questionId) => {
      return async (newOptionText) => {
        ...
        const question = findQuestionById(questions, questionId);
        setQuestions(
          getQuestionsForReplacedQuestionOptionsOfQuestion(
            questions,
            questionId,
            [...question.questionOptions, newQuestionOption]
          )
        );
      };
    },
  ...
  const getOnSetNewCorrectAnswerCallback = React.useCallback(
    (questionId) => {
  ...
  const getOnDeleteQuestionOptionFromQuestion = React.useCallback(
    (questionId) => {
  ...
  const getOnDeleteQuestionCallback = React.useCallback(
    (questionId) => {
      return async () => {
        await executeAsyncFunctionAndObserveState(
          setIsLoadingInProgress,
  ...
  const onElementMove = React.useCallback(
    async ({ sourceIndex, targetIndex }) => {
      setQuestions(
        getArrayAfterElementMove(questions, sourceIndex, targetIndex)
      );
      await executeAsyncFunctionAndObserveState(
        setIsLoadingInProgress,
        moveQuestions,
  ...
  const onSettingsSave = React.useCallback(
    async (testName, showResultAtTheEnd) => {
      onSettingsClose();
      await executeAsyncFunctionAndObserveState(
        setIsLoadingInProgress,
        updateTestSettings,
  ...
  return (
    <>
      ...
      {isLoadingInProgress ? <LoadingSpinner /> : null}
      <header>
        ...
        {testInfo ? (
          <h1 className="header__title fade-in" onClick={onSettingsPress}>
            EDITING TEST: "
            <span className="header-title__quiz-name">
              {testInfo.name || ""}
            </span>
            "
          </h1>
        ...
        />
        <button
          className="header__test-settings-button"
          onClick={onSettingsPress}
        >
          <img src="/static/settings.svg"></img>
          <span>Test Settings</span>
        </button>
        <button className="header__done-button" onClick={onDoneButtonPress}>
          Done
        </button>
      </header>
      <main>
          ...
          {questions.map((question, index) => (
            <GridElement key={question.id}>
              <Question
                ...
                onSetNewCorrectAnswer={getOnSetNewCorrectAnswerCallback(
                  question.id
                )}
                onDeleteQuestionOption={getOnDeleteQuestionOptionFromQuestion(
                  question.id
                )}
...
const TestSettingsDialog = ({
  ...
  const [
    isShowingOfFinalProcentEnabled,
    setIsShowingOfFinalProcentEnabled,
  ] = useState(initialIsShowingOfFinalProcentEnabled);

  const showingOfFinalProcentCheckBoxStateChangeCallback = React.useCallback(
    (evt) => {
  ...
  const [
    ...
    setIsCancellationConfirmationShown,
  ] = useState(false);
  ...
  const onCancelClick = React.useCallback(() => {
    ...
    if (wereChangesMade) setIsCancellationConfirmationShown(true);
    else onCancel();
  }, [
    initialTestName,
    testName,
    initialIsShowingOfFinalProcentEnabled,
    isShowingOfFinalProcentEnabled,
    onCancel,
  ]);
  ...
  const onDoneClick = React.useCallback(() => {
    const wereChangesMade =
      initialTestName !== testName ||
      initialIsShowingOfFinalProcentEnabled !== isShowingOfFinalProcentEnabled;
    if (wereChangesMade) onSave(testName, isShowingOfFinalProcentEnabled);
    else onCancel();
  }, [
    onSave,
    initialTestName,
  ...
  return (
    <>
      <GenericDialog title="Are you done?" onDismissDialog={onDismissDialog}>
        <span className="done-dialog__description">
          Your test can be accessed with entry-codes. Manage your entry-codes{" "}
          <a
            href={`/schools/${testId}/codes`}
            target="blank"
            className="u-color-inherit done-dialog-description-link"
          >
            here
          </a>
          .
